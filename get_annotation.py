...
class GetDataset:
    def __init__(self,video_dir,save_data_path,crop_size,hoop_pos,colume):
        ...
        self.colume = colume
        self.get_data()
        self.save_data()                                                                      ## Save the data as a whole .jpg file
...
class rough_seperation():
    
    def __init__(self,dir_pos, dir_neg, dataset, crop_size):
    ...
    def creater_annotation_file(self):
        ...
        if os.path.exists(self.save_annotation_dir):
            ...
            for node_i in range(len(self.label)):
                ...
                frame_id = node_i
                frame = ET.SubElement(nodes[1-label_i], self.title_list[0] + ' ' + str(frame_id))
...
fn_video = 'basketball-video-01.avi'
fn_data = 'data.jpg'
fn_annotation = 'annotation.xml'

video_dir = '.\\' + fn_video
save_data_path = '.\\' + fn_data
dir_pos = ".\\pos\\"
dir_neg = ".\\neg\\"
...
hoop_pos = hoop.return_pos()      #hoop_pos = [(924,133)]

#############################  Get Data  #########################################

data = GetDataset(video_dir,save_data_path,crop_size,hoop_pos,colume)   ## Get all data & save data as npy
dataset= data.return_data()

################################  Get Label  #####################################

rough_seperation(dir_pos, dir_neg, dataset, crop_size)
### Manually seperate the data after rough seperation 

###########################  Step 2 : Make Annotation File  ##############################

anno = create_annotation(dir_pos, dir_neg, fn_annotation, dataset, crop_size, colume)
# label = anno.return_()
